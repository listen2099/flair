...
def bed_to_psl(chromsizesfile, bed, outfilename):

    chromsizes = {}
    with open(chromsizesfile, 'r') as cf:
      for line in cf:
        line = line.rstrip().split('\t')
        chromsizes[line[0]] = line[1]
    cf.close()
    with open(outfilename, 'wt') as outfile, open(bed, 'r') as bf:
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(1)
    bed_to_psl(chromsizesfile, bed, outfilename)
